--window funcitions
--Инструкция PARTITION BY определяет столбец,
-- по которому данные будут делиться на группы,
-- ORDER BY определяет столбец, по которому значения внутри окна будут сортироваться
--ROWS и RANGE могут дополнительно задавать границы рамки окна
-- и ограничивать диапазон работы функций внутри партиции
